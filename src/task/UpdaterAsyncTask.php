...
class UpdaterAsyncTask extends AsyncTask {
    ...
    public function onCompletion() : void {
        ...
        if ($err !== null) {
            echo Terminal::$COLOR_RED . "[Sofia-Updater] Update notify error: $err" . Terminal::$COLOR_WHITE . PHP_EOL;
            return;
        }

        if ($highestVersion !== $this->pluginVersion) {
            echo Terminal::$COLOR_AQUA . "[Sofia-Updater] " . vsprintf("Version %s has been released for API %s. Download the new release at %s", ['v' . $highestVersion, $api, $artifactUrl]) . Terminal::$COLOR_WHITE . PHP_EOL;
        }
    }
}
